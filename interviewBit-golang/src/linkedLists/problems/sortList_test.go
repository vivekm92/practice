...
func TestMergeSortedLists(t *testing.T) {
	...
	for _, test := range tests {
		...
		result := MergeSortedLists(input1, input2)
		if !utils.CompareLinkedLists(result, test.output) {
			t.Errorf("MergeSortedLists(%v, %v) = %v; want %v", inputCompare1.String(), inputCompare2.String(), result.String(), test.output.String())
		}
	}
}
...
func TestSortList(t *testing.T) {
	tests := []struct {
		...
		{utils.GenerateLinkedList([]int{6, 5, 4, 3, 2, 1}), utils.GenerateLinkedList([]int{1, 2, 3, 4, 5, 6})},
		{utils.GenerateLinkedList([]int{1, 5, 4, 3}), utils.GenerateLinkedList([]int{1, 3, 4, 5})},
	}
	...
	for _, test := range tests {
		...
		if !utils.CompareLinkedLists(result, test.output) {
			t.Errorf("SortList(%v) = %v; want %v", inputCompare, result, test.output)
		}
	}
}
